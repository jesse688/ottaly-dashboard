...
async function previewSweep(db, { ukOnly = true, staleDays = 30 } = {}) {
  const { rows } = await db.query(
    `SELECT COUNT(*)::int AS total_domains,
            COUNT(*) FILTER (WHERE needs_check)::int AS to_check
       FROM (
         SELECT ${DOMAIN_NORM_SQL} AS domain,
                bool_or($1::int <= 0 OR ads_checked_at IS NULL
                        OR ads_checked_at < now() - ($1::int * interval '1 day')) AS needs_check
           FROM contacts
          WHERE ${scopeSql(ukOnly)}
          GROUP BY 1
       ) s
      WHERE ${VALID_DOMAIN}`, [staleDays]);
  return rows[0];
}
...
async function createSweep(db, { name, region = 'anywhere', ukOnly = true, staleDays = 30, cacheTtlDays = 7, auto = false } = {}) {
  ...
  await db.query(`UPDATE ads_batches SET total=$2 WHERE id=$1`, [id, rowCount]);

  if (!rowCount) {
    await db.query(`UPDATE ads_batches SET status='done' WHERE id=$1`, [id]);
    return { id, total: 0, cached: 0 };
  }
...
module.exports = { createSweep, previewSweep, getSetting, setSetting, UK_FILTER };

...
(async () => {
  ...
  app.get('/api/ch/location-values', requireSession, async (req, res) => {
    ...
    try {
      ...
      if (type === 'county') {
        ...
        const result = await db.query(`SELECT DISTINCT INITCAP(TRIM(county)) AS val FROM ch_companies ${where} ORDER BY val LIMIT 500`);
        rows = result.rows.map(r => r.val).filter(v => v && /^[A-Za-z]/.test(v));
      } else if (type === 'town') {
  ...
  // Pulls all warmup replies classified by the now-removed hyphen-pair heuristic
  // ("word-pair" in ai_reasoning), re-classifies them via Gemini, and returns
  // those that look genuinely interested. Safe read-only unless ?apply=1.
  //
  // GET  /api/admin/rescue-warmup          → scan + Gemini classify, return interested
  // POST /api/admin/rescue-warmup          → same, but also move them back to review

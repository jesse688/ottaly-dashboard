...
export default function HealthPage() {
  ...
  useEffect(() => {
    ...
    setFiltered(
      clients.filter((c) => (c.workspace_name ?? "").toLowerCase().includes(q)),
    );
  }, [clients, search]);

  const healthyCount = filtered.filter(
    (c) => c.snapshot?.health_band === "green",
  ).length;
  const warningCount = filtered.filter(
    (c) => c.snapshot?.health_band === "amber",
  ).length;
  const criticalCount = filtered.filter(
    (c) => c.snapshot?.health_band === "red",
  ).length;
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "1.75rem" }}>
        ...
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <div style={{ fontSize: "13px", color: "#6B7280", marginTop: "4px" }}>
            Daily AI-powered briefings per client
          </div>
        </div>
              ...
              if (!snap) {
                return (
                  ...
                  >
                    ...
                    >
                      No snapshot yet. The first daily build runs ~2 minutes
                      after the server starts and then every morning at 7am.
                    </div>
                  </div>
                );
              }
                  ...
                  <div style={{ padding: "0 1.25rem 1.25rem" }}>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {snap.reply_rate_7d != null
                            ? (snap.reply_rate_7d * 100).toFixed(1)
                            : "—"}
                          %
                        </div>
                      </div>
                      ...
                      >
                        ...
                        >
                          {snap.bounce_rate_7d != null
                            ? (snap.bounce_rate_7d * 100).toFixed(1)
                            : "—"}
                          %
                        </div>
                      </div>

...
export function UniboxClient({ companyName, clientName, clientEmail = '', workspaces = [], activeWorkspaceId }: { companyName: string; clientName: string; clientEmail?: string; workspaces?: Array<{ clientId: string; workspaceId: string; companyName: string }>; activeWorkspaceId?: string }) {
  ...
  // The lead whose async loads (thread + notes) are currently in flight. Every
  // fetch in openLead captures this id and, on resolve, bails if the user has
  // since opened a different lead — otherwise a slow response for lead A lands
  // on lead B's panel (you'd see B's contact card with A's conversation).
  ...
  function openLead(lead: Lead) {
    openLeadIdRef.current = lead.id   // mark this as the lead we're loading for
    setSelected(lead)
    setReplyMsg('')
    ...
    if (!lead.locked) {
      fetch(`/api/portal/leads/${lead.id}/data`).then(r => r.json()).then((d: { notes?: string | null }) => {
        if (openLeadIdRef.current !== lead.id) return   // user moved on — drop stale notes
        notesLoadedFor.current = lead.id
        setNotes(d?.notes ?? '')
      }).catch(() => { if (openLeadIdRef.current === lead.id) notesLoadedFor.current = lead.id })
    }
    ...
    fetch(`/api/portal/leads/${lead.id}/thread`).then(r => r.json()).then((d) => {
      // Ignore a thread that resolved after the user opened a different lead —
      // otherwise lead A's conversation renders under lead B (the core bug).
      if (openLeadIdRef.current !== lead.id) return
      setThread(Array.isArray(d) ? d : [])
    }).catch(() => { if (openLeadIdRef.current === lead.id) setThread([]) })
  }
  ...
  async function handleReply(text: string, html: string, to: string, cc: string, files: File[] = []) {
    ...
    // The client NEVER sees a failure. The reply route persists the message,
    // fires the live PlusVibe send, and ALWAYS notifies the team — including a
    // "⚠️ SEND MANUALLY" alert when live-send didn't fire. So a failure is the
    // ADMIN's to see and action, never the client's: a client-facing "Could not
    // send" only causes re-sends (double emails to the lead) and looks broken.
    // We fire the request, swallow any network/timeout error, and always show
    // "Reply sent". Errors are logged for us, not surfaced to the client.
    try {
      let res: Response
      if (files.length > 0) {
      ...
      if (!res.ok) console.error('[reply] non-OK response', res.status, '(admin notified; client shown success)')
    } catch (err) {
      console.error('[reply] request failed (admin notified; client shown success):', err)
    }
    setReplying(false)
    {
      ...
      // with the real persisted one, incl. attachment download ids). Skip if the
      // user has since opened a different lead — don't yank them back.
      const reconcileId = selected.id
      setTimeout(() => {
        if (openLeadIdRef.current === reconcileId && selected) {
...
function RichReply({ toEmail, ccEmail = '', placeholderName, sending, statusMsg, seed, onSend }: {
  ...
  // `ccEmail` is whoever the lead CC'd on their last inbound reply (may be several
  // comma-separated addresses). If the reply came WITH a CC, we include it by
  // default in the Cc field so the reply keeps that person (e.g. a looped-in
  // colleague) on the thread. The client can still remove/edit it before sending.
  ...
  // ccEmail arrives from the thread, which loads AFTER the composer mounts — so the
  // useState seeds above are empty on first render. Sync the Cc in once the inbound
  // CC resolves, unless the client has already edited Cc (so we never stomp a manual
  // change). Keyed on ccEmail so it also refreshes when switching leads.
